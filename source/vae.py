...
def plot(model, measure, mf, geodesic):
    ...
    ax2.scatter(data_points_reconstructed[:, 0], data_points_reconstructed[:, 1], marker=".", color="white", s=200,
                label='Encoded Demonstrations', alpha=0.1)
    ax2.plot(geodesic[0, :, 0], geodesic[0, :, 1], color="red")
...
def createVAEtrajectory(data):
    ...
    tests = auxiliary_tests.Tests()
    measure, mf, geodesic = tests.geodesic_computation(model, [test_traj], discrete_model=discrete_model)

    fig, ax = plt.subplots(figsize=(4, 4))

    pos = ax.imshow(np.rot90(mf), interpolation='bicubic',
               extent=[-latent_max, latent_max, -latent_max, latent_max])
    ...
    ax.scatter(data_points_reconstructed[:, 0], data_points_reconstructed[:, 1], marker=".", color="white", s=200,
                label='Encoded Demonstrations', alpha=0.1)
    ax.plot(geodesic[0, :, 0], geodesic[0, :, 1], color="red", label='Trajectory')
...
if __name__ == "__main__":
    # train = 0 anything else is just using model in models folder
    mode = 10
    ...
    current_letter = 'A'
    letters = ['S','A','C','J'] # Letters for testing
    # letters = ['S']
    filepath = "experiments/ur5_2/" # Folder for experiment 
    robot_data = True
    demo_path = 'Demos_ur5/' # Path for ur5 demonstrations
    #------------- Define Input Data ----------------------------#
...
# If stacked demonstrations is desired comment in below until next comment
...
# Until here
        ...
        # test_traj = train_data # Keep this in when doing 2D
        
# Code for testing two points in top layer

        # dems_top = pbddata.get_letter_data(letter=letter,n_samples=1,use_time=True)
        # data_top = [point for dem in dems_top for point in dem]

        # num_demos = 1  # Number of demonstrations
        ...
        #         new_row = np.append(row, z+rand)  # Add z as a new element
        #         result.append(new_row)  # No need for extra dimension
        # test_traj = [(np.array([row[0]]), row[1:]*0.1) for row in result]
        # test_traj = np.array(list(map(lambda x: x[1], test_traj)))
        # test_traj = np.pad(test_traj, ((0, 0), (0, 3)), mode='constant')
        # print(test_traj[0]) 
        # data = [(np.array([row[0]]), row[1:]*0.1) for row in data]
